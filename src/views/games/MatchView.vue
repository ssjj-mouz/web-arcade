...
<header>
    ...
    <div class="controls">
        ...
        <button onclick="location.NaN" style="background:#475569; margin-left:10px;">返回大厅</button>
    </div>
...
<div class="cmt-panel-header"><span>💬 深海寻宝 · 玩家评论</span><button onclick="toggleComments()">✕</button></div>
...
const symbols = ['🐟', '🐙', '🦀', '🐢', '🦈', '🐋', '🐡', '🦑'];
        ...
        // 读取大厅分数
        let highScore = localStorage.getItem('matchHighScore') || 999;
        if(highScore == 999) highScoreEl.innerText = '-';
        else highScoreEl.innerText = highScore;
        ...
        function gameOver() {
            setTimeout(() => {
                alert(`🎉 恭喜通关！共花费了 ${moves} 步。`);
                // 步数越少越好
                if(moves < highScore) {
                    if (typeof window._syncScore === 'function') _syncScore('match', moves);
                    highScoreEl.innerText = moves;
                }
            }, 500);
        }
